...
def select_folder():
    ...
    layout = [[sg.Text('功能说明：识别所选目录中照片的姿势', text_color="blue")],
              ...
              [sg.Text('选择目录', auto_size_text=True), sg.Input(size=(40, 1)), sg.FolderBrowse(
                  key='-Folder-', initial_folder=os.path.dirname(__file__))]
              ]
...
def capture_video(send_data):
    # For webcam input:
    valid_bones = ('NOSE', 'LEFT_SHOULDER', 'RIGHT_SHOULDER', 'LEFT_ELBOW',
    ...
    if send_data == True:
        ...
        client.connect(server_ip_port=(
            "192.168.0.103", 2000), keep_listen=False)
    ...
    with mp_pose.Pose(
        ...
        while cap.isOpened():
            ...
            if diff >= 0.05:
                print("->start_time: %s, end_time: %s, time diff: %s" %
                ...
                for i in range(33):
                    ...
                    # 世界坐标带缩放
                    x = results.pose_world_landmarks.landmark[mp_pose.PoseLandmark(
                        i).value].x * width
                    y = results.pose_world_landmarks.landmark[mp_pose.PoseLandmark(
                        i).value].y * height
                    z = results.pose_world_landmarks.landmark[mp_pose.PoseLandmark(
                        i).value].z * width
                    v = results.pose_world_landmarks.landmark[mp_pose.PoseLandmark(
                        i).value].visibility
