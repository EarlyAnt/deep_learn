...
def oper_sobel(file):
    ...
    dst1 = cv2.Sobel(img, cv2.CV_64F, dx=1, dy=0, ksize=3) # cv2的索贝尔算子
    dst2 = cv2.Sobel(img, cv2.CV_64F, dx=0, dy=1, ksize=3) # cv2的索贝尔算子
    # add = cv2.add(dst1, dst2) # 图像叠加方法一
    add = cv2.addWeighted(dst1, 0.9, dst2, 0.1, gamma=0) # 图像叠加方法二
